...
def tag_to_synset(tag):
    if not tag or tag in ["per", "loc", "org", "oth", "x", "w", "e", "num", "dat", "year", "bio"]:
        return None
    try:
        offset, pos = tag.rsplit("-", 1)
        return wn.synset_from_pos_and_offset(pos.lower(), int(offset))
    except:
        return None

def evaluate_my_results(gold_path, target_path):
    ...
    #only sids that exist in target (processed sentences)
    processed_sids = set(target.get("sent", {}).keys())

    for sid_str in processed_sids:
        gold_concepts = gold.get("conc", {}).get(sid_str, {})
        target_concepts = target["sent"][sid_str].get("concepts", {})

        for key, gold_item in gold_concepts.items():
            #gold_tag is from wids 
            gold_tag = None
            if gold_item.get("wids"):
                wid = gold_item["wids"][0]
                gold_tag = f"{wid:08d}-n"

            target_item = target_concepts.get(key)
            pred_tag = target_item.get("tag") if target_item else None

            if not pred_tag or pred_tag == "None":
                continue  #skip if no prediction

            total += 1
            tag_total[gold_tag or "unknown"] += 1

            if gold_tag == pred_tag:
                correct += 1
                tag_correct[gold_tag or "unknown"] += 1
                if gold_tag != "x":
                    correct_no_x += 1
                    total_no_x += 1
            else:
    ...
    print(f"\nEvaluation for {target_path}")
    if total > 0:
        print(f"Total accuracy: {correct / total:.2%} ({correct}/{total})")
    else:
        print("No concepts available for comparison")

    if total_no_x > 0:
        print(f"Accuracy without 'x': {correct_no_x / total_no_x:.2%} ({correct_no_x}/{total_no_x})")
    else:
        print("No concepts without 'x'")

#evaluation for 4 context sizes

gold_file = "twwtn-en_human (1).json"

for i in range(20):
    target_file = f"twwtn-en_human (1)_tagged_context{i}.json"
    if os.path.exists(target_file):
        evaluate_my_results(gold_file, target_file)
    else:
